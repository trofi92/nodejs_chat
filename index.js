...
let user_id = 0; /**클라이언트에게 부여되는 고유값 */
let ALL_WS = []; /**전체 유저 통제, 웹소켓과 아이디 저장 */
ws.on ("connection", function connect(websocket,req){  /**웹소켓에 특정 클라이언트가 연결 되었을때 실행*/
    user_id++;
    ...
    function sendUserId(user_id) {
        ...
        websocket.send(JSON.stringify(data)); /** 웹소켓 : 서버와 클라이언트 간 데이터 교환의 매개 =>*/
    }
});
